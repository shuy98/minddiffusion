...
class Decoder(nn.Cell):
    def __init__(self, *, ch, out_ch, ch_mult=(1, 2, 4, 8), num_res_blocks,
        ...
        self.mid.attn_1 = make_attn(
            block_in, attn_type=attn_type, dtype=self.dtype)
        ...
        for i_level in reversed(range(self.num_resolutions)):
            ...
            for i_block in range(self.num_res_blocks+1):
                ...
                if curr_res in attn_resolutions:
                    attn.append(
                        make_attn(block_in, attn_type=attn_type, dtype=self.dtype))
            ...
            curr_res = curr_res * 2
            up.update_parameters_name(
                prefix=self.param_prefix + f"up.{i_level}.")
            if len(self.up) != 0:
                self.up.insert(0, up)
            else:
                self.up.append(up)
